...
**Container + config:** For provider overrides and custom endpoints, see the operational note in [`config-reference.md`](/docs/reference/api/config-reference) (custom `api_url` vs `ZEROCLAW_PROVIDER`).

---

## CLI command map

Full command surface (flags and subcommands): [`commands-reference.md`](/docs/reference/cli/commands-reference).
...
**Drift warning:** New releases may add or gate tools. Confirm in source or by inspecting the live tool list (for example [`zeroclaw mcp serve`](/docs/mcp-serve) and JSON-RPC `tools/list` for an allowlisted surface).

**Agent loop:** [`[agent]`](/docs/reference/api/config-reference) keys include `max_tool_iterations`, `parallel_tools`, `tool_call_dedup_exempt`, **`tool_filter_groups`** (MCP tool schemas), and optional **`[agent.tool_router]`** (small model narrows visible tools per user message).

Extending the codebase: [`change-playbooks.md`](/docs/contributing/change-playbooks) (“Adding a Tool”).
...
Use **`zeroclaw mcp serve`** to expose a **curated allowlist** of ZeroClaw tools over MCP (`2024-11-05`). Full detail: [`mcp-serve.md`](/docs/mcp-serve).
...
External MCP servers attach tools with names you typically match via patterns (e.g. `mcp_vikunja_*`). Control which schemas reach the model per turn with **`[[agent.tool_filter_groups]]`** in [`config-reference.md`](/docs/reference/api/config-reference).
...
| Mechanism | Where |
|-----------|--------|
| CLI | `zeroclaw skills list`, `install`, `remove`, `audit` — see [`commands-reference.md`](/docs/reference/cli/commands-reference) (`skills` section). |
| Config | `[skills]` and optional `skills_directory` for scoped loading — [`config-reference.md`](/docs/reference/api/config-reference). |
...
| Mechanism | Role |
|-----------|------|
| **`[autonomy]`** | `level` (`read_only` / `supervised` / `full`), workspace and path rules, `allowed_commands`, `require_approval_for_medium_risk`, `block_high_risk_commands`, **`auto_approve`**, **`always_ask`**, budgets. See [`config-reference.md`](/docs/reference/api/config-reference) (section `[autonomy]`). |
| **`zeroclaw estop`** | Emergency stop levels including **tool freeze** for named tools — [`commands-reference.md`](/docs/reference/cli/commands-reference). Requires `[security.estop].enabled` where applicable. |
