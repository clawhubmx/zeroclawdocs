...
```bash
curl -fsSL https://raw.githubusercontent.com/morpheum-labs/zeroclaw/master/install.sh | bash
```
...
it pulls `ghcr.io/morpheum-labs/zeroclaw:latest` and tags it locally before running.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/morpheum-labs/zeroclaw:latest`)
- `--skip-install`
- `--provider <id>`
